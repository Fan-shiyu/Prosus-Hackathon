...
## Hard-won lessons — avoid these traps

- "More staff fixes walkouts": Walkouts on zero-cover days are noise — customers never arrived. Staff costs doubled and bankrupted us faster. Read the signal before acting.
- "Optimize the walkout band": Walkouts are downstream. Stockouts → walkouts → ghost reviews → reputation collapse → demand collapse. Fix the cause, not the symptom.
- "Cheapest supplier saves money": Italian Imports Co. was 30% cheaper on pasta but delivers Wednesday-only. Order Wednesday, get pasta *next* Wednesday. Effective lead time, not price, is the metric.
- "Reputation collapsed from unhappy customers": It collapsed because no customers were served at all on weekends. Model the arithmetic before the sentiment.
- "Throw an LLM at the whole loop": Day 13 bankruptcy. LLM ordered from "FreshFishCo" and "Supplier A" — invented names. LLMs hallucinate structured data. Only use exact names from the observation.
- "Prompt engineer away hallucinations": Whack-a-mole. Fix supplier names, it invents ingredient names. Fix those, quantities fall below min_order. Validate at boundaries, not in prose.
- "Variance is unavoidable": Most variance was the same root cause on different days. Variance usually means the mechanism isn't isolated yet.
- "Bigger safety buffer fixes stockouts": Buffer + Wednesday-only delivery + 0.85 reliability = still stockout on Saturday. Buffers only help if the calendar math is right.
- "Keep tuning what's working": Biggest score jumps came from changing *what* the agent decides, not *how well*. Know when to stop polishing and start adding.
- "Remove the LLM when it hurts baseline": Removing it loses tourist_season +9k and supply_crisis +12k. Constrain *when* it runs — don't disable it entirely."""
